...
class BeneficiariesSummariesView(APIView):
    ...
    def get(self, request):
        ...
        if district_id:
            ...
            total_beneficiaries_count = 0
            county_beneficiaries = []
            for county in counties:
                ...
                county_details["beneficiaries"] = beneficiaries_count
                county_beneficiaries.append(county_details)
                total_beneficiaries_count += beneficiaries_count
            district_serializer = DistrictSerializer(district)
            res = {"total_beneficiaries": total_beneficiaries_count, "district": district_serializer.data,
                   "counties": county_beneficiaries}
            return Response(res, status=status.HTTP_200_OK)
        if county_id:
            county = None
            try:
            ...
            for sub_county in sub_counties:
                ...
                sub_county_details["beneficiaries"] = beneficiaries_count
                sub_county_beneficiaries.append(sub_county_details)
                total_beneficiaries_count += beneficiaries_count
            county_serializer = CountySerializer(county)
            res = {"total_beneficiaries": total_beneficiaries_count, "county": county_serializer.data,
                   "sub_counties": sub_county_beneficiaries}
            return Response(res, status=status.HTTP_200_OK)
        if subcounty_id:
            subcounty = None
            try:
            ...
            total_beneficiaries_count = 0
            parish_beneficiaries = []
            for parish in parishes:
                villages = Village.objects.filter(parish_id__in=parishes)
                beneficiaries_count = CommunityOrganization.objects.filter(village_id__in=villages).count()
                parish_serializer = ParishSerializer(parish)
                parish_details = parish_serializer.data
                parish_details["beneficiaries"] = beneficiaries_count
                parish_beneficiaries.append(parish_details)
                total_beneficiaries_count += beneficiaries_count
            subcounty_serializer = SubCountySerializer(subcounty)
            res = {"total_beneficiaries": total_beneficiaries_count, "subcounty": subcounty_serializer.data,
                   "parishes": parish_beneficiaries}
            return Response(res, status=status.HTTP_200_OK)
        if parish_id:
            parish = None
            try:
            ...
            total_beneficiaries_count = 0
            village_beneficiaries = []
            for village in villages:
                beneficiaries_count = CommunityOrganization.objects.filter(village_id=village.id).count()
                village_serializer = VillageSerializer(village)
                village_details = village_serializer.data
                village_details["beneficiaries"] = beneficiaries_count
                village_beneficiaries.append(village_details)
                total_beneficiaries_count += beneficiaries_count
            parish_serializer = ParishSerializer(parish)
            res = {"total_beneficiaries": total_beneficiaries_count, "parish": parish_serializer.data,
                   "villages": village_beneficiaries}
            return Response(res, status=status.HTTP_200_OK)
        else:
    ...
    def get(self, request):
        ...
        if district_id:
            ...
            for county in counties:
                ...
                county_totals.append({"county": county_serializer.data, "total_savings": county_savings_total})
                total_district_savings += county_savings_total

            district_serializer = DistrictSerializer(district)
            res = {
                "district": district_serializer.data,
                "total_savings": total_district_savings,
        ...
        if county_id:
            ...
            for subcounty in subcounties:
                ...
                subcounty_totals.append(
                    {"subcounty": sub_county_serializer.data, "total_savings": subcounty_savings_total}
                )
            ...
            res = {
                ...
                "total_savings": total_county_savings,
        ...
        if subcounty_id:
            ...
            for parish in parishes:
                ...
                parish_totals.append({"parish": parish_serializer.data, "total_savings": parish_savings_total})
                subcounty_total += parish_savings_total

            sub_county_serializer = SubCountySerializer(subcounty)
            res = {
                "subcounty": sub_county_serializer.data,
                "total_savings": subcounty_total,
        ...
        if parish_id:
            ...
            for village in villages:
                ...
                savings = Saving.objects.filter(farmer_id__in=farmers)
                village_savings_total = savings.aggregate(Sum("amount")).get("amount__sum") if savings else 0
                village_totals.append({"village": village_serializer.data, "total_savings": village_savings_total})
                parish_total += village_savings_total

            parish_serializer = ParishSerializer(parish)
            res = {"parish": parish_serializer.data, "total_savings": parish_total, "villages": village_totals}
            return Response(res, status=status.HTTP_200_OK)
        ...
        for district in districts:
            ...
            district_totals.append({**district_serializer.data, "total_savings": district_savings_total})
            total_national_savings += district_savings_total

        res = {"total_savings": total_national_savings, "districts": district_totals}
        return Response(res, status=status.HTTP_200_OK)
