...
class AddUserAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if User.objects.filter(email=request.data.get("email")).exists() is True:
            res = {"msg": "User with the email {} already registered.".format(request.data.get("email")), "data": None,
                   "success": False}
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                send_mail_to_admin = send_credentials(
                    'Welcome onboard {}'.format(department.name),
                    client_message,
                    new_user.email
                )
                ...
                res = {"msg": "You have successfully registered. Check your email for login information",
                       "data": ser.data, "success": True}
...
class UserLoginAPIView(generics.CreateAPIView):
        ...
        def post(self, request, format=None):
            ...
            if check_password(request.data["password"], user.password):
                ...
                res = {"msg": "Login success", "success": True,
                       "data": {"name": fullname, "username": user.username, "id": user.id, "token": token,
                                "roles": roles, "phone": user.phone}}
                return Response(data=res, status=status.HTTP_200_OK)
